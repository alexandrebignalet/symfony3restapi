Feature: Access CronSurvey data through API
  ...
  As a client user
  I need to be able to retrieve JSON encoded CronSurvey resources.
  ...
  Scenario: User can GET a Collection of CronSurvey objects
    ...
    """
  Scenario: User can GET a CronSurvey object by Id
  ...
  Scenario: User cannot modify CronSurvey's data
    When I send a "PUT" request to "/cronsurveys/cs1" with body:
    """
    {
      "survey": {"id": "s1"},
      "hourMin": "2016-08-17 06:00:00",
      "hourMax": "2016-08-17 11:00:00",
      "everyday": true
    }
    """
  ...
  Scenario: User cannot delete a CronSurvey
    When I send a "DELETE" request to "/cronsurveys/cs1"
    Then the response code should be 403

  Scenario: User cannot GET a none existent CronSurvey
    When I send a "GET" request to "/cronsurveys/cs100"
    Then the response code should be 403

  Scenario: User cannot PUT a none existent CronSurvey
